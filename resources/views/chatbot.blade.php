...
<div id="chat-window">
    ...
    <div id="chat-register" style="padding:20px;flex:1">
        <h3 style="margin-bottom:10px">{{ __('messages.chatbot.register.title') }}</h3>
        <p style="font-size:13px;color:#555;margin-bottom:14px">{{ __('messages.chatbot.register.desc') }}</p>

        <input id="first_name" placeholder="{{ __('messages.chatbot.register.first_name') }}" style="width:100%;padding:12px;border-radius:12px;border:1px solid #ddd;margin-bottom:8px">
        <input id="last_name" placeholder="{{ __('messages.chatbot.register.last_name') }}" style="width:100%;padding:12px;border-radius:12px;border:1px solid #ddd;margin-bottom:8px">
        <input id="email" placeholder="{{ __('messages.chatbot.register.email') }}" style="width:100%;padding:12px;border-radius:12px;border:1px solid #ddd;margin-bottom:8px">
        <input id="mobile" placeholder="{{ __('messages.chatbot.register.mobile') }}" style="width:100%;padding:12px;border-radius:12px;border:1px solid #ddd;margin-bottom:8px">
    ...
    <div id="chat-box" style="display:none;flex-direction:column;height:100%">
        ...
        <div style="padding:12px;border-top:1px solid #eee;background:white;display:flex;gap:8px">
            ...
            <button id="send-btn" style="background:#2563eb;color:white;border:none;width:42px;height:42px;border-radius:50%"><i class="fas fa-paper-plane"></i></button>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(userToken && userName){
        ...
        addMessage(`Halo <b>${userName}</b>! Silakan tanyakan jadwal penerbangan ✈️`, 'bot');
    }
        ...
        try {
            const res = await fetch('/api/chat/register',{
                ...
                headers:{
                    ...
                    'Accept':'application/json'  // FIX: pastikan Laravel balikin JSON
                },
            ...
            addMessage("Maaf, registrasi gagal. Silakan coba lagi.", 'bot');
        }
    };
    ...
    try {
        ...
        if(res.status === 422){
            ...
            addMessage(
              'Session kamu sudah tidak valid. Silakan daftar ulang.',
              'bot'
            );
            return;
        }
        ...
        if(data.reply){
            ...
            addMessage('AI tidak mengirim balasan','bot');
        }
