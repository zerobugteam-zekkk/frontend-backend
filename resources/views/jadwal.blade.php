...
<head>
    ...
    <style>
        ...
        .status-on-time { color: #059669; background: #ecfdf5; border: 1px solid #d1fae5; }
        .status-delay { color: #dc2626; background: #fef2f2; border: 1px solid #fee2e2; }
        .status-boarding { color: #2563eb; background: #eff6ff; border: 1px solid #dbeafe; animation: pulse-blue 2s infinite; }
        .status-arrived { color: #1e40af; background: #e0e7ff; border: 1px solid #c7d2fe; }
        .status-departed { color: #64748b; background: #f1f5f9; border: 1px solid #e2e8f0; }
    ...
    <nav class="fixed top-0 w-full z-50 bg-slate-950/80 backdrop-blur-md border-b border-white/5">
        <div class="container mx-auto px-6 py-4 flex items-center justify-between">
            <a href="{{ url('/') }}" class="group flex items-center space-x-4">
                <div class="bg-blue-600 p-2 rounded-lg group-hover:bg-blue-500 transition-all duration-300 shadow-lg shadow-blue-600/20">
            ...
            <div class="flex items-center space-x-8">
                ...
                <div class="flex flex-col text-right border-l border-white/10 pl-8">
                    <span id="header-time" class="text-xl font-black mono-font text-white leading-none tracking-tighter italic">00:00:00</span>
...
<header class="relative min-h-[90vh] flex flex-col justify-center bg-slate-950 font-formal overflow-hidden">
    ...
    <div class="container mx-auto px-6 relative z-10">
        ...
        <div class="max-w-4xl pt-32 pb-32">
            {{-- Kicker Badge --}}
            <div class="inline-flex items-center gap-3 mb-8 px-4 py-1.5 border border-blue-400/20 rounded-full bg-blue-500/5 backdrop-blur-md" data-aos="fade-right">
            ...
            {{-- Tabs & Search Area --}}
            <div class="flex flex-col md:flex-row items-start md:items-center gap-6 mt-12" data-aos="fade-up" data-aos-delay="300">
                ...
                <div class="relative w-full md:w-80 group">
                    <i class="fas fa-search absolute left-5 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-blue-500 transition-colors"></i>
                    <input type="text" id="flightSearch" placeholder="{{ __('messages.schedule.search.placeholder') }}"
...
<style>
    ...
    <main class="container mx-auto px-6 -mt-16 relative z-30 pb-20">
        <div class="bg-white rounded-[2.5rem] shadow-2xl shadow-slate-900/10 border border-slate-100 overflow-hidden" data-aos="fade-up">
        ...
        <div class="mt-8 flex flex-col md:flex-row justify-between items-center bg-slate-900 rounded-[2rem] p-8 text-white gap-6 shadow-xl border border-white/5" data-aos="fade-up">
            <div class="flex items-center space-x-6">
                <div class="w-14 h-14 bg-blue-600/20 rounded-2xl flex items-center justify-center border border-blue-600/20">
        ...
        <section class="mt-28 border-t border-slate-200 pt-20">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-16">
                ...
                <div class="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-12 text-slate-500 text-sm leading-relaxed">
                    <div class="space-y-6">
                    ...
                    <div class="space-y-8">
                        ...
                        <div class="bg-slate-100 p-8 rounded-[2rem] border border-slate-200 shadow-inner">
                            <ul class="space-y-4 font-bold text-slate-800 uppercase text-[9px] tracking-[0.15em]">
                                <li class="flex items-center"><i class="fas fa-check-circle text-blue-600 mr-4 text-sm"></i> {{ __('messages.schedule.info.bullets.1') }}</li>
                                <li class="flex items-center"><i class="fas fa-check-circle text-blue-600 mr-4 text-sm"></i> {{ __('messages.schedule.info.bullets.2') }}</li>
                                <li class="flex items-center"><i class="fas fa-check-circle text-blue-600 mr-4 text-sm"></i> {{ __('messages.schedule.info.bullets.3') }}</li>
                            </ul>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        async function loadFlights(force = false) {
            ...
            try {
                ...
                document.getElementById('table-body').innerHTML = `<tr><td colspan="6" class="p-20 text-center text-red-500 font-bold">${T.error_load}</td></tr>`;
            }
        }
        ...
        function renderData() {
            ...
            document.getElementById('last-update').innerText = new Date().toLocaleTimeString(JS_LOCALE, {hour:'2-digit', minute:'2-digit'});
        }
        ...
        function updateHeaderClock() {
            ...
            document.getElementById('header-date').innerText = now.toLocaleDateString(JS_LOCALE, { weekday:'long', day:'numeric', month:'short', year:'numeric' }).toUpperCase();
        }
